...
function triggerExplosion(x, y) {
    ...
    for (let i = zombies.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(z.x - x, z.y - y) < 140) {
            z.hp -= 100;
            // Removed z.flashTimer assignment to stop explosion hit flashing
        }
    }
    updateHUD();
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = zombies.length - 1; j >= 0; j--) {
            ...
            if (Math.hypot(b.x - z.x, b.y - z.y) < z.radius) { 
                z.hp -= 5; 
                // Removed z.flashTimer assignment to stop bullet hit flashing
...
function draw() {
    ...
    zombies.forEach(z => {
        ...
        // Flash logic removed: Always use normal color mapping
