...
// Added function to check collision with existing items to prevent overlap
...
function update() {
    ...
    // Pickups loops - Bombs Detonation Activation Logic
    for (let i = bombs.length - 1; i >= 0; i--) {
        let b = bombs[i];
        let triggered = false;
        
        // Check if player steps on it
        if (Math.hypot(player.x - b.x, player.y - b.y) < 30) {
            triggered = true;
        } else {
            // Check if any zombie steps on it
...
function spawnItem(type) {
    ...
    while (s < 50) {
        ...
        // Added checkItemCol to prevent items spawning on top of each other
