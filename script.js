...
bgImage.src = 'https://raw.githubusercontent.com/divanshu911/New-things/736c8aca12961f3145a8257b1efde09b8e704130/IMG_GRASS911.jpg';
...
// Function to check collision with existing items to prevent overlap
...
function update() {
    ...
    if(mx !== 0 || my !== 0) {
        ...
        // PLAYER CORNER SLIP LOGIC
        // If walking straight vertically and blocked, check if we can slide horizontally
        ...
        // If walking straight horizontally and blocked, check if we can slide vertically
    ...
    for (let i = bombs.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(player.x - b.x, player.y - b.y) < 30) {
            ...
            for (let j = 0; j < zombies.length; j++) {
                if (Math.hypot(zombies[j].x - b.x, zombies[j].y - b.y) < 30) {
                    triggered = true;
                    break;
                }
            }
    ...
    zombies.forEach((z, idx) => {
        ...
        if (canMoveX && canMoveY) {
            ...
            // Blocked vertically, channel speed into sliding horizontally
            z.x += vx; 
            z.x += (vx >= 0 ? z.speed : -z.speed) * 0.5; 
        } else if (!canMoveX && canMoveY) {
            // Blocked horizontally, channel speed into sliding vertically
            z.y += vy;
            z.y += (vy >= 0 ? z.speed : -z.speed) * 0.5; 
        } else {
            // Blocked on both sides (inner corner), wiggle around to unstuck
            z.x += (Math.random() - 0.5) * z.speed;
            z.y += (Math.random() - 0.5) * z.speed;
        }
        
        // Zombie-to-Player damage check
...
function draw() {
    ...
    // 1. Image Background Draw
    // Stretches the image to fit the whole canvas window
    ...
    // 3. Drop Collectibles
    bombs.forEach(b => {
        ctx.beginPath(); ctx.arc(b.x, b.y, 10, 0, Math.PI * 2); ctx.fillStyle = '#111'; ctx.fill();
        ctx.fillStyle = '#ff3333'; ctx.fillRect(b.x - 2, b.y - 14, 4, 6);
    });
    medkits.forEach(m => {
    ...
    // Updated Powerups with geometrical textures
    powerups.forEach(p => {
        ...
        ctx.strokeStyle = '#fff';
        if (p.type === 'shield') {
            // Shield Texture: A plus symbol
            ctx.fillRect(p.x - 4, p.y - 1.5, 8, 3);
            ctx.fillRect(p.x - 1.5, p.y - 4, 3, 8);
        } else if (p.type === 'speed') {
            // Speed Texture: Forward chevrons
            ctx.beginPath(); 
            ctx.moveTo(p.x - 3, p.y - 3); ctx.lineTo(p.x + 1, p.y); ctx.lineTo(p.x - 3, p.y + 3); 
            ctx.moveTo(p.x + 1, p.y - 3); ctx.lineTo(p.x + 5, p.y); ctx.lineTo(p.x + 1, p.y + 3);
            ctx.lineWidth = 2; ctx.stroke();
        } else {
            // Ammo Texture: Three small bullet slots
            ctx.fillRect(p.x - 4.5, p.y - 3, 2, 6);
            ctx.fillRect(p.x - 1, p.y - 3, 2, 6);
            ctx.fillRect(p.x + 2.5, p.y - 3, 2, 6);
        }
    });
    ...
    zombies.forEach(z => {
        ...
        ctx.fillStyle = z.isBoss ? '#27ae60' : '#4E704D'; // Boss Green vs Normal Green
        ctx.fill();
        ctx.strokeStyle = '#1e3f20'; ctx.lineWidth = 2; ctx.stroke();
        
        // Infection eye spots to show look direction
        let ang = Math.atan2(player.y - z.y, player.x - z.x);
    ...
    ctx.fillRect(0, -4, player.radius + 12, 8);
    
    // Player Circle Body
    ctx.beginPath();
    ctx.arc(0, 0, player.radius, 0, Math.PI * 2);
    ctx.fillStyle = '#3498db'; // Active Player Blue
    ctx.fill();
...
function spawnItem(type) {
    ...
    while (s < 50) {
        ...
        // Check checkItemCol to prevent items spawning on top of each other
        if (!checkBuildingCol(rx, ry, 20) && !checkItemCol(rx, ry, 20)) {
            placed = true;
            break;
        }
        s++;
