...
function update() {
    ...
    for (let i = bombs.length - 1; i >= 0; i--) {
        ...
        if (!b.ignited) {
            ...
            // If touched by player or zombie, start the countdown fuse
            if (triggered) {
                b.ignited = true;
            }
        } else {
            // Bomb has been ignited! Count down the fuse ticks
            b.fuse--;
            if (b.fuse <= 0) {
                bombs.splice(i, 1); 
...
function draw() {
    ...
    bombs.forEach(b => {
        ...
        if (b.ignited && Math.floor(b.fuse / 5) % 2 === 0) {
            ctx.fillStyle = '#ff3333'; // Flashing Danger Red
        } else {
            ctx.fillStyle = '#111';    // Default Black Body
        }
...
function spawnItem(type) {
    ...
    // Initialize with ignition status and default fuse duration
...
// --- MODIFIED TO WIPE BOMBS ARRAY CLEAN ON RESTART ---
function startGame() {
