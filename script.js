...
function update() {
    ...
    for (let i = powerups.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(player.x - p.x, player.y - p.y) < 30) {
            ...
            if (p.type === 'freeze') player.freezeTimer = 720; // Extended active frame timer (12 seconds)
    ...
    zombies.forEach((z, idx) => {
        ...
        // Disable forward tracking forces if freeze powerup engine is active
        ...
        // Turn off push scaling routines during freeze to keep zombies statically aligned
    ...
    if (gameState === 'PLAYING') {
        // IMPORTANT: Only count down if the freeze timer is NOT active
...
function draw() {
    ...
    powerups.forEach(p => {
        ...
        // Deep blue shading implementation specifically for distinct freeze powerup coloring
        ...
        if (p.type === 'shield') {
            ...
            // Visual multi-axis geometric snowflake matrix design layout (*)
            ctx.beginPath();
    ...
    zombies.forEach(z => {
        ...
        // Renders zombies in a chilly frosty color scheme while frozen
        if (player.freezeTimer > 0) {
...
function updateHUD() {
    ...
    if (timerElement) {
        ...
        // IMPORTANT: Add "Icy" CSS styling when freeze powerup is active
        if (player.freezeTimer > 0) {
            timerElement.style.backgroundColor = '#a5f2ff'; // Light icy blue
            timerElement.style.color = '#0033aa';           // Deep blue text
            timerElement.style.boxShadow = '0 0 10px cyan'; // Glow effect
            timerElement.style.borderRadius = '5px';
            timerElement.style.padding = '2px 8px';
        } else {
            // Reset to defaults when the freeze ends
...
function startGame() {
    ...
    // Clear active active powerup effects and timers when starting a fresh game session
