...
function update() {
    ...
    // ADDED: Pickups loops - Bombs Detonation Activation Logic (Now includes Zombies)
...
function draw() {
    ...
    // 3. Drop Collectibles (Replaced glitchy text with Vector shapes)
    bombs.forEach(b => {
    ...
    // 4. Enemy Rendering (Replaced broken symbol with 2D Circle Graphics)
    zombies.forEach(z => {
        ctx.beginPath();
    ...
    // 6. Player Geometry Layout (Replaced raw text weapon emoji with custom circular body + gun barrel extension)
    ctx.save(); 
