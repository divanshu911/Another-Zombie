...
function reload() {
    ...
    setTimeout(() => { if(gameState === 'PLAYING' && !isPaused) { player.ammo = player.maxAmmo; player.isReloading = false; updateHUD(); } }, 1200);
}

// System execution state manager method for switching pause configurations
...
function draw() {
    ...
    bombs.forEach(b => {
        ...
        if (b.ignited && Math.floor(b.fuse / 4) % 2 === 0) {
...
// Mouse Click Event Binding for top right Floating Pause Button Interface
document.getElementById('pauseBtn').addEventListener('click', (e) => {
    e.stopPropagation(); // Stops mouse tracking mechanics from throwing a bullet downstream
    togglePause();
});

// Mobile Tapping Event Binding for top right Floating Pause Button Interface
