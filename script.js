...
function triggerExplosion(x, y) {
    // NOTE: Audio trigger was safely removed from here to prevent duplicate playback loops.
    screenShake = 15;
...
function update() {
    ...
    for (let i = bombs.length - 1; i >= 0; i--) {
        ...
        if (!b.ignited) {
            ...
            if (triggered) {
                b.ignited = true;
                playSound(sounds.explosion); // Audio track plays instantly upon setting off the trap!
            }
        } else {
            b.fuse--;
            if (b.fuse <= 0) {
...
function draw() {
    ...
    // --- BOMBS RENDERING LAYER WITH FLASH ANIMATION ---
    bombs.forEach(b => {
        ctx.beginPath(); ctx.arc(b.x, b.y, 10, 0, Math.PI * 2); 
        
        // Fast-paced strobe flash setup based on the remaining fuse
...
function spawnItem(type) {
    ...
    // Fixed configuration: Set fuse counter directly to 45 frames
...
// Window Event Wrapper ensures DOM structures exist before running resize matrices
